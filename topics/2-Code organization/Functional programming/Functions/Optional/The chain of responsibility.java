...
class ChainOfResponsibilityDemo {
    ...
    static RequestHandler commonRequestHandler = wrapInRequestTag
            .compose(createDigest.compose(wrapInTransactionTag));
    ...
    interface RequestHandler {
        // it allows using lambda expressions for creating handlers below
        Request handle(Request request);

        default RequestHandler compose(RequestHandler before) {
            Objects.requireNonNull(before);
            return (Request req) -> handle(before.handle(req));
        }

        default RequestHandler andThen(RequestHandler after) {
            Objects.requireNonNull(after);
            return (Request req) -> after.handle(handle(req));
        }
    }
